 ...
 * Engine that uses a true logging backend.
 * <p>
 * The factory isn't dedicated to be used by client code but by the
 * {@link org.silverpeas.kernel.logging.SilverLogger} class to obtain an instance of a logger
 ...
 * <p>
 * The bind between the {@link org.silverpeas.kernel.logging.SilverLoggerFactory} interface and its
 * implementation is performed by the Java SPI (Java Service Provider Interface). Only the first
 * available logger factory implementation is loaded. If no implementation candidate is found
 * through the Java SPI, then a default implementation, that uses the standard Java Logging
 * mechanism, is loaded.
 *
 * @author mmoquillon
 */
public interface SilverLoggerFactory {
   ...
   * Provider Interface and this implementation wraps the concrete logging mechanism used by the
   * Silverpeas Logging Engine. If no candidate is found through the Java SPI, then a default
   * implementation is loaded; this default implementation wraps the Java Logging system.
   *
  ...
  /**
   * Gets a {@link org.silverpeas.kernel.logging.SilverLogger} instance for the specified namespace.
   ...
   * found for the given logger namespace. If no level setting is found from the configuration or if
   * there is no configuration found for the specified namespace, then the logger level is set to
   * null meaning it should inherit its level from its nearest ancestor with a specific (non-null)
   ...
   * {@link org.silverpeas.kernel.logging.SilverLogger#getLogger(String)} method or by the
   ...
   * hierarchical relationships between the loggers from the root one.
   ...
   * configuration. If no level setting is found from the configuration or if there is no
   * configuration found for the specified namespace, then the logger level is set to null meaning
   * it should inherit its level from its nearest ancestor with a specific (non-null) level value.
   * It is the responsibility of the implementation of the logger to take care of the logging level
   * inheritance and of the default log handlers/adapters used by Silverpeas.
   * <p>
   * This method should not be invoked directly. It is dedicated to be used by the
   * {@link org.silverpeas.kernel.logging.SilverLogger#getLogger(String)} method or by the
   * implementation of the Silverpeas Logging Engine.
   ...
   * @param namespace the hierarchical dot-separated namespace of the logger mapping the
   * hierarchical relationships between the loggers from the root one.
   * @param configuration the logger configuration to use when initializing the manufactured logger.
   * If the logger already exists, the configuration won't be used in order to avoid any replacement
   * of the existing configuration. To update its configuration, please use instead
   * {@link org.silverpeas.kernel.logging.LoggerConfigurationManager}.
   * @return a Silverpeas logger instance.
   */
