 ...
 * a specific Silverpeas module (a business set of features) of which Silverpeas is made up. Each
 * logger is defined by a namespace that is a dot-separated logger hierarchy and a level that
 * filters the kind of messages to record into the Silverpeas logs. If the level of a logger isn't
 * set explicitly, then its level will be its nearest ancestor with a non-null logging level. It
 * will be the first ancestor with a non-null logging level that will take in charge of the
 * recording of the messages from its child loggers to the logs.
 * <p>
