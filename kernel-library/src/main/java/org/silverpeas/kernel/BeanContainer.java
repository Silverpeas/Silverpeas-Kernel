 ...
 * interface will be used. The expectation here is the IoC subsystem should implement the JSR-330 and as such it should
 * support IoD; examples of such subsystems are Spring, Micronaut or CDI (JSR-365). Moreover, the bean container imposes
 * an additional constrain to ease the use of IoD in Silverpeas whatever the underlying implementation: the dependencies
 * shouldn't be resolved through constructor or method parameters but instead by using the {@link javax.inject.Inject}
 * annotation on fields or by using a {@link ManagedBeanProvider} instance. Keep in mind to leave your code agnostic
 * from any specific code of an IoC solution so it should be more easy later to change such a solution.
 * <p>
 * The goal of the bean container is to manage the life-cycle of some objects (named here beans), to resolve and to
 * satisfy automatically and in a transparent way their dependencies. For doing, the classes for which the instances
 * have to be managed by the container should be elective for life-cycle management and registered into this container.
 * The way the election and the registration of theses classes are performed is the responsibility of the underlying IoC
 * subsystem.
 *
 * @author mmoquillon
