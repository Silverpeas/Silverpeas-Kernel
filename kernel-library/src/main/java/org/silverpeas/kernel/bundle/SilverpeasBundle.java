 ...
 * of Silverpeas. Each data is defined by a pair of key-value whose key identifies the data.
 * @author mmoquillon
 */
public interface SilverpeasBundle {
   ...
   * @return a list of values, each of them as String. If the property of an object to read exists but
   ...
   *   </ul>
   * If the computed key isn't defined in the bundle, then no {@link MissingResourceException}
   * exception is thrown (for compatibility reason with Silverpeas versions lesser than 6) but an empty list is
   * returned.
   * @param list the identifier of the list in the bundle.
   * @param property the object's property for which the value will be fetch.
   ...
   * specified property of the first max objects are read (if the property isn't set for an object,
   * it is set to an empty string).
   * @return an a list of string with one property value (if max is 1) or with several values (if
