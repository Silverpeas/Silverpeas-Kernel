...
@EnableSilverTestEnv
@TestManagedBeans({MyBean1.class, MyBean3.class})
class ManagedBeansTest {
  ...
  @Test
  @Order(3)
  void theBeansAreWellInjected() {
    assertThat(testedBean.getMyBean(), notNullValue());
    assertThat(testedBean.getMyBean(), instanceOf(MyBean1.class));
    assertThat(testedBean.getMyBean().getFoo(), is(MyBean1.DEFAULT_FOO));
  }

  @Test
  void theBeansAreManagedInTheBeanContainer() {
    MyBean1 myBean1 = beanProvider.getManagedBean(MyBean1.class);
    MyBean3 myBean3 = beanProvider.getManagedBean(MyBean3.class);
    assertThat(myBean1.getFoo(), is(MyBean1.DEFAULT_FOO));
    assertThat(myBean3.getMyBean1(), is(myBean1));
  }
}
